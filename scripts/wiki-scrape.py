...
# this finds all the spans with the mw-headline class, which is all of the
# main genre titles
...
# for future uses of this code the number of valid categories needs to be
# confirmed, or other means of exclusion added
for mainGenre in mainGenres[0:20]:
    # the key and genreList will populate our dictionary
    key = (mainGenre.text.lower())
    genreList = []
    # the id of each mainGenres span is used to target the list of bullet
    # points that follow them
    ...
    try:
        ...
        for li in genres.find_all('li'):
            ...
            # minor issue here that where a string was initially split, double
            # spacing is created
            allText = li.get_text()
            # where there are sub-genres, each is added as it's own li element
            # this prevents them being added twice from the parent genre li
            # element
            if '\n' in allText:
            ...
            # the join and split here is getting rid of any double spaces
                ...
                genreList.append(' '.join(allText.lower().split()))
    except:
        pass
